...
if(isset($_POST['isi_baslat']))
{
    ...
    header("Location: /index.php?url=makina_is_ekran&planlama-id={$planlama_id}&makina-id={$makina_id}");
    exit;
...
if(isset($_POST['bakim_bitir'])){
    ...
    if($bakim_sorunu_cozuldumu){
        ...
        header("Location: /index.php?url=makina_listesi");
    }
    exit;
}
...
if(isset($_POST['aktar'])){
    ...
    if(empty($sonraki_asama_olan_planlama)) //Sonraki Aşamada Yoksa
    {
        ...
        header("Location: /index.php?url=makina_is_ekran&planlama-id={$son_eklenen_planlama_id}&makina-id={$makina_id}");
    }
    else //Sonraki Aşamada Daha Önce Varsa
    {
        ...
        header("Location: /index.php?url=makina_is_ekran&planlama-id={$mevcut_planlama['id']}&makina-id={$makina_id}");
    }
...
//Real-Time Veri Güncelleme (Her 5 saniyede bir çağrılır)
if(isset($_GET['islem']) && $_GET['islem'] == 'realtime-veri'){
    try {
        $planlama_id = intval($_GET['planlama_id']);
        $makina_id = intval($_GET['makina_id']);
        $grup_kodu = isset($_GET['grup_kodu']) ? $_GET['grup_kodu'] : '';
        ...
        // 1. Toplam Üretilen Adet
        $sql = "SELECT SUM(uretilen_adet) as toplam FROM uretilen_adetler WHERE planlama_id = :id";
        $sth = $conn->prepare($sql);
        $sth->bindParam('id', $planlama_id);
        $sth->execute();
        $calisma = $sth->fetch(PDO::FETCH_ASSOC);
        $toplam_uretilen_adet = intval($calisma['toplam']);

        // 2. Planlama Bilgisi ve Üretim Aktarılan Adet
        $sql = "SELECT durum, grup_kodu, mevcut_asama FROM planlama WHERE id = :id";
        $sth = $conn->prepare($sql);
        $sth->bindParam('id', $planlama_id);
        $sth->execute();
        $planlama = $sth->fetch(PDO::FETCH_ASSOC);
        
        $durum = $planlama['durum'];
        
        // Üretim aktarılan adet
        ...
        $sth->bindParam('aktarilan_asama', $planlama['mevcut_asama']);
        $sth->execute();
        $aktarilan = $sth->fetch(PDO::FETCH_ASSOC);
        
        $uretim_aktarilan_adet = intval($aktarilan['aktarilan_adet']);
        ...
        // 3. Makinadaki İş Sayısı (makinalar JSON kolonunda)
        $sql = "SELECT makinalar, mevcut_asama FROM planlama 
        ...
        foreach ($planlama_isler as $planlama_is) {
            $makinalar = json_decode($planlama_is['makinalar'], true);
            if(isset($makinalar[$planlama_is['mevcut_asama']]) && $makinalar[$planlama_is['mevcut_asama']] == $makina_id) {
                $makinadaki_is_sayisi++;
            }
        }

        // 4. Okunmamış Mesaj Sayısı (şimdilik devre dışı - tablo yapısı farklı)
        $mesaj_sayisi = 0;
        ...
        echo json_encode([
            'success' => true,
            'toplam_uretilen_adet' => $toplam_uretilen_adet,
            'kalan_adet' => $kalan_adet,
            'makinadaki_is_sayisi' => $makinadaki_is_sayisi,
            'durum' => $durum_text,
