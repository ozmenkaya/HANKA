...
{ 
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) as count FROM kod3 WHERE firma_id = ? AND aciklama = ?");
    ...
    if ($result['count'] > 0) {
        ...
        header('Location: /index.php?url=kod3');
        die();
    }
    
    $sql = "INSERT INTO kod3(firma_id, aciklama, olusturan_id, olusturma_tarihi) VALUES(?, ?, ?, ?);";
...
if(isset($_GET['islem']) && $_GET['islem'] == 'kod_sil')
{
    ...
    $sql = "DELETE FROM kod3 WHERE id = :id AND firma_id = :firma_id";
    $sth = $conn->prepare($sql);
    $sth->bindParam('id', $id);
...
if ($islem === 'get_kod' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Kod bilgilerini çek
    $stmt = $conn->prepare("SELECT id, aciklama FROM kod3 WHERE id = ? AND firma_id = ?");
...
if ($islem === 'kod_guncelle' && isset($_POST['kod_guncelle'])) {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) as count FROM kod3 WHERE aciklama = ? AND id != ? AND firma_id = ?");
    ...
    // Kod güncelle
    $stmt = $conn->prepare("UPDATE kod3 SET aciklama = ?, guncelleme_tarihi = ?, guncelleyen_id = ? WHERE id = ? AND firma_id = ?");
