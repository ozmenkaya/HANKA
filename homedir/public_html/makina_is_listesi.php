...
?>
                                    ...
                                    <tbody>
                                        ...
                                        <?php foreach ($isler as $is) { ?>
                                            <?php 
                                                ...
                                                // CACHE kullan (100x daha hızlı!)
                                                $uretim_aktarilan_adet = isset($aktarilan_cache[$is['grup_kodu']][$is['mevcut_asama']]) 
                                                    ? $aktarilan_cache[$is['grup_kodu']][$is['mevcut_asama']]
                                                    : (isset($orijinal_adetler[$is['mevcut_asama']]) ? $orijinal_adetler[$is['mevcut_asama']] : 0);

                                                $uretim_aktarilan_adet = $uretim_aktarilan_adet == 0 ? 1 : $uretim_aktarilan_adet;

                                                $toplam_uretilen_adet = isset($uretilen_cache[$is['grup_kodu']][$is['mevcut_asama']]) 
                                                    ? $uretilen_cache[$is['grup_kodu']][$is['mevcut_asama']]
                                                    : 0;
                                                //print_r($adetler);
                                                            ...
                                                            <li class="list-group-item d-flex gap-2">
                                                                ...
                                                                <?php if($is['mevcut_asama'] < $is['asama_sayisi'] ){ ?>
                                                                    <?php 
                                                                        ...
                                                                        // CACHE kullan
                                                                        $departman_adi = isset($departman_cache[$mevcut_departman_id]) ? $departman_cache[$mevcut_departman_id] : "Bilinmiyor";
                                                                    ?>
                                                                    
                                                                    <span class="badge bg-secondary mb-1"> <?php echo $departman_adi; ?>  </span>
                                                                <?php }else{ ?>
        ...
        <script>
            ...
            setTimeout(verilerıGuncelle, 15000);
            
            // Her 10 saniyede bir güncelle
            setInterval(verilerıGuncelle, 30000);
