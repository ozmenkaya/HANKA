...
?>
            ...
            <div class="card border-secondary mt-2 mb-1">
                <div class="card-header border-secondary d-md-flex align-items-center justify-content-between pt-0 pb-0">
                    ...
                    <h5>
                        ...
                        <span class="badge bg-secondary" style="border-radius:50%" data-realtime="makinadaki_is_sayisi"> 
                                    ...
                                    <ol class="list-group mt-3" style="margin-top:4px!important">
                                        <li class="list-group-item table-info2"> 
                                            ...
                                            <span
                                                ...
                                                data-realtime="toplam_uretilen_adet"
                                                style="cursor:pointer"
                                            >
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                            <div class="row mb-1">
                                <div class="col-md-6">
                                    ...
                                    <input type="number" id="labelWidth" class="form-control" value="<?php echo $etiket_genislik; ?>" step="1">
                                </div>
                                <div class="col-md-6">
                                    <label for="labelHeight" class="form-label">Yükseklik (cm):</label>
                                    <input type="number" id="labelHeight" class="form-control" value="<?php echo $etiket_yukseklik ?>" step="1">
                                </div>
                            </div>
                            <div class="row mb-2">
                                <div class="col-md-6">
                            ...
                            <div id="label-content" class="label-container" style="width: <?php echo $etiket_genislik; ?>cm; height: <?php echo $etiket_yukseklik; ?>cm;">
                    ...
                    <div class="modal-footer"> 
                        ...
                        <button type="submit" class="btn btn-blue waves-effect waves-light" id="saveAsPdf">Pdf</button>                       
                    </div>
                </div>
            </div>
        </div>
        ...
        <script>
            $(document).ready(function(){
                ...
                // REAL-TIME UPDATE: Her 5 saniyede bir veri güncelle (sayfa yenilemeden)
                let sonGuncelleme = Date.now();

                // Real-time veri güncelleme (5 saniyede bir)
                function verilerıGuncelle() {
                    const makina_id = <?php echo $makina_id; ?>;
                    
                    // Güncelleme göstergesi
                    $('#realtime-status').removeClass('bg-success').addClass('bg-warning');
                    
                    $.ajax({
                        url: '/index.php?url=makina_is_ekran_db_islem&islem=realtime-veri',
                        method: 'GET',
                        data: {
                            planlama_id: planlama_id,
                            makina_id: makina_id,
                            grup_kodu: grup_kodu
                        },
                        dataType: 'json',
                        success: function(data) {
                            if(data.success) {
                                // Üretilen adet güncelle (number format ile)
                                if(data.toplam_uretilen_adet !== undefined) {
                                    const formattedUretilen = data.toplam_uretilen_adet.toLocaleString('tr-TR');
                                    $('[data-realtime="toplam_uretilen_adet"]').text(formattedUretilen);
                                }
                                
                                // Kalan adet güncelle (number format ile)
                                if(data.kalan_adet !== undefined) {
                                    const formattedKalan = data.kalan_adet.toLocaleString('tr-TR');
                                    $('[data-realtime="kalan_adet"]').text(formattedKalan);
                                }
                                
                                // Makinadaki iş sayısı güncelle
                                if(data.makinadaki_is_sayisi !== undefined) {
                                    $('[data-realtime="makinadaki_is_sayisi"]').text(data.makinadaki_is_sayisi);
                                }
                                
                                // Durum güncelle
                                if(data.durum !== undefined) {
                                    $('[data-realtime="durum"]').text(data.durum);
                                }
                                
                                // Mesaj sayısı güncelle
                                if(data.mesaj_sayisi !== undefined && data.mesaj_sayisi > 0) {
                                    $("#mesajlar span").text(data.mesaj_sayisi);
                                } else if(data.mesaj_sayisi === 0) {
                                    $("#mesajlar span").text('');
                                }
                                
                                // Son güncelleme zamanı
                                sonGuncelleme = Date.now();
                                console.log('✓ Veriler güncellendi:', new Date().toLocaleTimeString(), data);
                                
                                // Başarılı güncelleme göstergesi (yeşil)
                                $('#realtime-status').removeClass('bg-warning bg-danger').addClass('bg-success');
                            }
                        },
                        error: function(xhr, status, error) {
                            ...
                            try {
                                ...
                                console.error('Ham Hata:', xhr.responseText);
                            }
                            // Hata göstergesi (kırmızı)
                            $('#realtime-status').removeClass('bg-success bg-warning').addClass('bg-danger');
                        }
                    });
                }
                
                // İlk yüklemede hemen çalıştır
                setTimeout(verilerıGuncelle, 2000);
                
                // Her 5 saniyede bir güncelle
                setInterval(verilerıGuncelle, 5000);
