<?php 
       $sth = $conn->prepare('SELECT * FROM para_birimleri');
       $sth->execute();
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header" style="padding-top:10px;padding-bottom:10px">
                    <h5 class="modal-title" id="pbModalLabel">Para Birimleri</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body ptop-8 pbtm-8">
                    <!-- Arama Kutusu -->
                    <div class="search-container mb-1">
                        <input type="text" class="form-control" id="searchInputPb" placeholder="Ara...">
                        <span class="search-icon">🔍</span>
                    </div>
                    <!-- Ölçü Birimleri Tablosu -->
                    <table class="table table-striped table-bordered table-sm table-hover" id="pbTable">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            <?php
                                foreach ($birimler as $birim) {
                                    echo '<tr class="pb-row" data-id="' . $birim['id'] . '">';
                                    echo '<td class="text-center ptop-5 pbtm-5">' . $birim['id'] . '</td>';
                                    echo '<td class="text-center ptop-5 pbtm-5">' . $birim['kod'] . '</td>';
