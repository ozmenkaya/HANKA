...
function processWhatsAppMessage($conn, $firma_id, $message) {
    ...
    // Makina işleri sorgusu - "X makinasında en yüksek adetli iş"
    if (preg_match('/(makina|makinası|makinasında).*(en\s+yüksek|en\s+büyük|maksimum|max).*(adet|iş)/i', $message) ||
        preg_match('/(omega|kba|hd|hotmelt|laminasyon).*(en\s+yüksek|en\s+büyük).*(adet|iş)/i', $message)) {
        
        // Makina adını çıkar
        $makina_pattern = '';
        if (preg_match('/(omega|kba|hd|hotmelt|laminasyon|hot\s*melt)/i', $message, $matches)) {
